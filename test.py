...
class TestInteractiveMathGame(unittest.TestCase):
    ...
    def setUp(self):
        self.valid_numbers = [3, 1, 2, 5]
        self.invalid_expression = "3 + 2 == "  # 修正无效表达式，确保格式符合解析要求
        self.valid_expression = "3 + 1 == 4"
        self.unsupported_expression = "3 + 1 == 4 + 2"

    @patch('EquationMaster.generate_numbers', return_value=[3, 1, 2, 5])
    ...
    @patch('EquationMaster.generate_numbers', return_value=[3, 1, 2, 5])
    @patch('builtins.input', side_effect=['4', '1', '10', '3 + 1 == 4', 'no', 'no'])
    ...
    @patch('EquationMaster.generate_numbers', return_value=[3, 1, 2, 5])
    @patch('builtins.input', side_effect=['4', '1', '10', '3 + 2 ==', '3 + 2 ==', '3 + 2 ==', 'yes', 'no', 'no'])
    @patch('sys.stdout', new_callable=StringIO)
    ...
    @patch('EquationMaster.generate_numbers', return_value=[3, 1, 2, 5])
    @patch('builtins.input', side_effect=['yes', 'no', 'maybe', 'no', 'yes', 'no', 'yes', 'no', 'no'])
    @patch('sys.stdout', new_callable=StringIO)
    def test_play_again_prompt(self, mock_stdout, mock_input, mock_generate_numbers):
        ...
        with patch('sys.exit', side_effect=SystemExit):
            try:
                ...
                pass
        output = mock_stdout.getvalue()
        self.assertIn("Invalid input. Please enter 'yes' or 'no'.", output)
